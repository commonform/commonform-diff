...
module.exports = function commonformdiff (a, b) {
  // The diff algorithm is run on stringified forms, so its edit
  // operation values for additions and replacements will be
  // stringified. Destringify them here.
  var patch = diff(a, b)
  ...
  patch.forEach(function (operation) {
    ...
    if (op === 'remove') {
      ...
      // If the operation is to remove an entire heading, mark every
      // word in the heading as deleted.
...
// Return the array index of the nth content element that has _not_ been
// marked for deletion with `{ deleted: true }`.
